...
public class PinyinHelper {
	private static Properties PINYIN_TABLE = null;
	private static Properties MUTIL_PINYIN_TABLE = null;
	// 拼音分隔符
	...
	private static Properties getPinyinTable(Context context) {
		if (null == PINYIN_TABLE) {
	...
	private static Properties getMultiPinyinTable(Context context) {
		if (null == MUTIL_PINYIN_TABLE) {
			MUTIL_PINYIN_TABLE = PinyinResource.getMutilPintinTable(context);
		}

		return MUTIL_PINYIN_TABLE;
	}
		...
		for (int i = pinyinArray.length - 1; i >= 0; i--) {
			...
			for (int j = originalPinyin.length() - 1; j >= 0; j--) {
				...
				if (originalChar < 'a' || originalChar > 'z') {
					...
					pinyinArray[i] = originalPinyin.replaceAll(String.valueOf(originalChar),
	 ...
	 * @return 汉字的拼音
	 */
	public static String[] convertToPinyinArray(Context context, char c, PinyinFormat pinyinFormat) {
		String pinyin = getPinyinTable(context).getProperty(String.valueOf(c));
	 ...
	 * @return 字符串的拼音
	 */
	public static String[] convertToPinyinArray(Context context, char c) {
		return convertToPinyinArray(context, c, PinyinFormat.WITH_TONE_MARK);
	}
	 ...
	 * @return 字符串的拼音
	 */
	public static String convertToPinyinString(Context context, String str, String separator, PinyinFormat pinyinFormat) {
		str = ChineseHelper.convertToSimplifiedChinese(context, str);
		...
		for (int i = 0, len = str.length(); i < len; i++) {
			...
			if (ChineseHelper.isChinese(c) || c == '〇') {
				...
				for (int rightIndex = (i + rightMove) < len ? (i + rightMove) : (len - 1); rightIndex > i; rightIndex--) {
					...
					if (getMultiPinyinTable(context).containsKey(cizu)) {
						String[] pinyinArray = formatPinyin(getMultiPinyinTable(context).getProperty(cizu),
								pinyinFormat);
						assert pinyinArray != null;
				...
				if (!isFoundFlag) {
					String[] pinyinArray = convertToPinyinArray(context, str.charAt(i), pinyinFormat);
					if (pinyinArray != null) {
	 ...
	 * @return 转换后带声调的拼音
	 */
	public static String convertToPinyinString(Context context, String str, String separator) {
		return convertToPinyinString(context, str, separator, PinyinFormat.WITH_TONE_MARK);
	}
	 ...
	 */
	public static boolean hasMultiPinyin(Context context, char c) {
		String[] pinyinArray = convertToPinyinArray(context, c);
	 ...
	 * @return 对应拼音的首字母
	 */
	public static String getShortPinyin(Context context, String str) {
		// 使用#作为拼音分隔符
		String separator = "#"; 
		StringBuilder sb = new StringBuilder();
		...
		for (int i = 0, len = str.length(); i < len; i++) {
			...
			if (!ChineseHelper.isChinese(c) && c != '〇') {
				...
				String hanziPinyin = convertToPinyinString(context, sb.toString(), separator, PinyinFormat.WITHOUT_TONE);
